...
cl_int parse_options(int argc, char * argv[], struct cl_sdk_options_Diagnostic * diag_opts,
    struct cl_sdk_options_SingleDevice * dev_opts, struct options_Binaries * binaries_opts)
{
    ...
    opts = tmp;
    MEM_CHECK(tmp  = add_CLI_options(opts, &n, BinariesOptions,     CAG_ARRAY_SIZE(BinariesOptions)),     error, end);
    opts = tmp;
    ...
    while (cag_option_fetch(&cag_context)) {
        ...
        PARS_OPTIONS(parse_BinariesOptions(identifier, &cag_context, binaries_opts), state);
...
prg:    OCLERROR_RET(clReleaseProgram(program), end_error, cont);
        ...
        OCLERROR_PAR(kernel = cl_util_read_text_file(kernel_location, &program_size, &error), error, cont);

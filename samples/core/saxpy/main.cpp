...
int main(int argc, char* argv[])
{
    try
    {
        ...
        auto prng = [engine = std::default_random_engine{},
                     dist = std::uniform_real_distribution<cl_float>{ -100.0, 100.0 }]() mutable {
            return dist(engine);
        };

        cl_float a = prng();
        std::valarray<cl_float> arr_x(length),
                                arr_y(length);
        ...
        // Validate
        if (std::equal(std::begin(arr_x), std::end(arr_x),
                       std::begin(arr_y), std::end(arr_y)))

...
void Conway::initializeGL()
{
    ...
    auto create_shader = [](std::string file_path, cl_GLenum shader_stage)
    {
        ...
        glShaderSource(shader, 1, &pshader_string, NULL); checkError("glShaderSource(shader, 1, &pshader_string, NULL)");
        glCompileShader(shader); checkError("glCompileShader(shader)");

        GLint status = GL_FALSE;
        glGetShaderiv(shader, GL_COMPILE_STATUS, &status); checkError("glGetShaderiv(shader, GL_COMPILE_STATUS, &status)");
        if(status != GL_TRUE)
        {
            int log_length = 0;
            glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &log_length); checkError("glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &log_length)");
            std::vector<GLchar> shader_log(log_length);
            glGetShaderInfoLog(shader, log_length, NULL, shader_log.data()); checkError("glGetShaderInfoLog(shader, log_length, NULL, shader_log.data())");
    ...
    auto create_program = [](std::initializer_list<GLuint> shader_stages)
    {
        ...
        for (auto shader_stage : shader_stages) {
            glAttachShader(program, shader_stage); checkError("glAttachShader(program, shader_stage)"); }

        glLinkProgram(program); checkError("glLinkProgram(program)");
        GLint status = GL_FALSE;
        glGetProgramiv(program, GL_LINK_STATUS, &status); checkError("glGetProgramiv(program, GL_LINK_STATUS, &status)");
        if(status != GL_TRUE)
        {
            int log_length = 0;
            glGetProgramiv(program, GL_INFO_LOG_LENGTH, &log_length); checkError("glGetProgramiv(program, GL_INFO_LOG_LENGTH, &log_length)");
            std::vector<GLchar> program_log(log_length);
            glGetProgramInfoLog(program, log_length, NULL, program_log.data()); checkError("glGetProgramInfoLog(program, log_length, NULL, program_log.data())");
    ...
    glGenBuffers(1, &vertex_buffer); checkError("glGenBuffers(1, &vertex_buffer)");
    glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer); checkError("glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer)");
        glBufferData(GL_ARRAY_BUFFER, quad.size() * sizeof(float), quad.data(), GL_STATIC_DRAW); checkError("glBufferData(GL_ARRAY_BUFFER, quad.size() * sizeof(float), quad.data(), GL_STATIC_DRAW)");
    glBindBuffer(GL_ARRAY_BUFFER, 0); checkError("glBindBuffer(GL_ARRAY_BUFFER, 0)");

    glGenVertexArrays(1, &vertex_array); checkError("glGenVertexArrays(1, &vertex_array)");
    glBindVertexArray(vertex_array); checkError("glBindVertexArray(vertex_array)");
        glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer); checkError("glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer)");
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 4*sizeof(float), (GLvoid *)(NULL)); checkError("glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 4*sizeof(float), (GLvoid *)(NULL))");
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4*sizeof(float), (GLvoid *)(NULL + 2 * sizeof(float))); checkError("glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 4*sizeof(float), (GLvoid *)(NULL + 2 * sizeof(float)))");
        glEnableVertexAttribArray(0); checkError("glEnableVertexAttribArray(0)");
        glEnableVertexAttribArray(1); checkError("glEnableVertexAttribArray(1)");
    ...
    std::generate_n(std::back_inserter(texels),
        ...
        [prng = std::ranlux48{ std::random_device{}() },
         dist = std::uniform_int_distribution<std::uint16_t>{ 0, 1 }]() mutable
    {
        return static_cast<GLchar>(dist(prng));
    });

    glUseProgram(gl_program); checkError("");
    for(auto image : {&gl_images.front, &gl_images.back})
    {
        glGenTextures(1, image); checkError("glGenTextures(1, image);");
        glActiveTexture(GL_TEXTURE0); checkError("glActiveTexture(GL_TEXTURE0);");
        glBindTexture(GL_TEXTURE_2D, *image); checkError("glBindTexture(GL_TEXTURE_2D, *image);");
            glTexImage2D(GL_TEXTURE_2D, 0, GL_R8UI, getSize().x, getSize().y, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE, texels.data()); checkError("glTexImage2D(GL_TEXTURE_2D, 0, GL_R8UI, getSize().x, getSize().y, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE, texels.data())");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); checkError("glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); checkError("glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); checkError("glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);");
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); checkError("glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);");
        glBindTexture(GL_TEXTURE_2D, 0); checkError("glBindTexture(GL_TEXTURE_2D, 0);");
    }
    glUseProgram(0); checkError("glUseProgram(0);");

    glViewport(0, 0, getSize().x, getSize().y); checkError("glViewport(0, 0, getSize().x, getSize().y)");
    glDisable(GL_DEPTH_TEST); checkError("glDisable(GL_DEPTH_TEST)");
}
...
{
    auto conway = cl::KernelFunctor<cl::ImageGL, cl::ImageGL, cl_float2>{cl_program, "conway"};
    ...
    conway(
        cl::EnqueueArgs{
            queue,
            cl::NDRange{ getSize().x, getSize().y }
        },
        cl_images.front,
        cl_images.back,
        cl_float2{
...
void Conway::render()
{
    ...
    if (!cl_khr_gl_event_supported) glFinish();
    else glFlush();
}

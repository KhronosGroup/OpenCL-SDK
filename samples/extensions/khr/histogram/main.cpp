...
struct HistogramOptions { size_t length; size_t bins;  };
...
template <> auto cl::sdk::parse<HistogramOptions>(){
    return std::make_tuple(
        ...
        std::make_shared<TCLAP::ValueArg<size_t>>("b", "bins", "Bins of histogram", false, 100, "positive integral")
    );
}
template <> HistogramOptions cl::sdk::comprehend<HistogramOptions>(
    std::shared_ptr<TCLAP::ValueArg<size_t>> length_arg,
    std::shared_ptr<TCLAP::ValueArg<size_t>> bins_arg)
{
    return HistogramOptions{
        length_arg->getValue(), bins_arg->getValue()
    };
}
    ...
    try
    {
        ...
        if (!diag_opts.quiet) {
            std::cout <<
                ...
                "Selected local memory size: " << device.getInfo<CL_DEVICE_LOCAL_MEM_SIZE>() << "\n" <<
                std::endl;
        }
        ...
        auto histogram_shared = cl::KernelFunctor<
            cl_uint, cl_uint, cl_uint, cl::Buffer, cl::Buffer, cl::LocalSpaceArg, cl::Buffer
        >(program, "histogram_shared");
        auto histogram_global = cl::KernelFunctor<
            cl_uint, cl_uint, cl::Buffer, cl::Buffer, cl::Buffer
        >(program, "histogram_global");
        ...
        auto prng = [engine = std::default_random_engine{},
                     dist = std::uniform_real_distribution<cl_float>{ min, max }]() mutable {
            return dist(engine);
        };
        ...
        cl::Buffer buf_input{ context, std::begin(input), std::end(input), true },
                   ...
                   buf_histogram{ context, std::begin(histogram), std::end(histogram), false };
        ...
        {
            histogram_global(
                cl::EnqueueArgs{ queue, cl::NDRange{ length } },
                length, bins, buf_input, buf_levels, buf_histogram
            );
        }
        ...
        for(const auto value : input)
        {
            ...
            {
                const auto bin_iter = std::upper_bound(
                    std::begin(levels), std::end(levels), value
                );
        ...
        // Fetch results
        cl::copy(queue, buf_histogram, std::begin(histogram), std::end(histogram));

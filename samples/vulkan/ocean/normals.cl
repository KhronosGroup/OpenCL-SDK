     ...
     read_only image2d_t src, write_only image2d_t dst )
{
    ...
    float dz_c = read_imagef(src, sampler, fuv).g;
    float dz_cr = read_imagef(src, sampler, (float2)(fuv.x + texel, fuv.y)).g;
    float dz_ct = read_imagef(src, sampler, (float2)(fuv.x, fuv.y + texel)).g;
    float dz_cl = read_imagef(src, sampler, (float2)(fuv.x - texel, fuv.y)).g;
    float dz_cb = read_imagef(src, sampler, (float2)(fuv.x, fuv.y - texel)).g;
    float dz_tr = read_imagef(src, sampler, (float2)(fuv.x + texel, fuv.y + texel)).g;
    float dz_br = read_imagef(src, sampler, (float2)(fuv.x + texel, fuv.y - texel)).g;
    float dz_tl = read_imagef(src, sampler, (float2)(fuv.x - texel, fuv.y + texel)).g;
    float dz_bl = read_imagef(src, sampler, (float2)(fuv.x - texel, fuv.y - texel)).g;

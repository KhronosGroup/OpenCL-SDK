...
{
    return device.getInfo<CL_DEVICE_OPENCL_C_VERSION>().find(version_fragment) != cl::string::npos;
}

bool cl::util::supports_extension(const cl::Device& device, const cl::string& extension)
{
    return device.getInfo<CL_DEVICE_EXTENSIONS>().find(extension) != cl::string::npos;
}
...
bool cl::util::supports_feature(const cl::Device& device, const cl::string& feature_name)
{
    ...
    return std::find_if(c_features.cbegin(), c_features.cend(), feature_is_work_group_reduce) != c_features.cend();
}
#endif

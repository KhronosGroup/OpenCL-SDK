...
#define IF_ERR(func, er, label) \
do { if (func) {*error = er; goto label;} } while (0)
...
do { if (func) {*error = er; goto label;} } while (0)
    ...
    while (1) {
        if (used + READALL_CHUNK + 1 > size) {
            ...
            MEM_CHECK(temp = (unsigned char *)realloc(data, size), *error, dt);
            data = temp;
        }
    ...
    MEM_CHECK(temp = (unsigned char *)realloc(data, used), *error, dt);
    data = temp;
    if (length != NULL)
        *length = used;
    ...
    const char * const program_file_name, cl_int * error)
{
    ...
    for (cl_uint i = 0; i < num_devices; ++i) {
        ...
        if (*error == CL_INVALID_VALUE) {
            fprintf(stderr, "No suitable file found\n");
            goto end;
        };
    }

    OCLERROR_PAR(program = clCreateProgramWithBinary(context, num_devices, devices,
        binaries_size, binaries_ptr, error, NULL), *error, end);

...
UTILS_EXPORT
char * cl_util_read_text_file(const char * const filename, size_t * const length, cl_int * const error)
{
...
do { if (func) { err = error_type; goto label; } } while (0)
...
UTILS_EXPORT
unsigned char * cl_util_read_binary_file(const char * const filename, size_t * const length, cl_int * const error)
{
    cl_int err = CL_SUCCESS;
...
UTILS_EXPORT
cl_int cl_util_write_binaries(const cl_program program, const char * const program_file_name)
{
    ...
    OCLERROR_RET(clGetProgramInfo(program, CL_PROGRAM_NUM_DEVICES, sizeof(cl_uint), &num_devices, NULL), error, end);
    if (num_devices == 0)
        return CL_INVALID_PROGRAM_EXECUTABLE;

    // read the binaries size
    binaries_size_alloc_size = sizeof(size_t) * num_devices;
    MEM_CHECK(binaries_size = (size_t *)malloc(binaries_size_alloc_size), error, end);

    OCLERROR_RET(clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES, binaries_size_alloc_size, binaries_size, NULL), error, end);

    // read the binaries
    binaries_ptr_alloc_size = sizeof(unsigned char *) * num_devices;
    MEM_CHECK(binaries_ptr = (unsigned char **)malloc(binaries_ptr_alloc_size), error, end);
    memset(binaries_ptr, 0, binaries_ptr_alloc_size);

    for (cl_uint i = 0; i < num_devices; ++i)
        MEM_CHECK(binaries_ptr[i] = (unsigned char *)malloc(binaries_size[i]), error, end);

    OCLERROR_RET(clGetProgramInfo(program, CL_PROGRAM_BINARIES, binaries_ptr_alloc_size, binaries_ptr, NULL), error, end);

    // query devices
    MEM_CHECK(devices = (cl_device_id *)malloc(sizeof(cl_device_id) * num_devices), error, end);
    OCLERROR_RET(clGetProgramInfo(program, CL_PROGRAM_DEVICES, sizeof(cl_device_id) * num_devices, devices, NULL), error, end);

    // write the binaries to files
    for (cl_uint i = 0; i < num_devices; ++i) {
        // get device name
        char name_data[201];
        OCLERROR_RET(clGetDeviceInfo(devices[i], CL_DEVICE_NAME, sizeof(name_data) - 1, name_data, NULL), error, end);
        name_data[200] = '\0';
        ...
        if (f != NULL) {
            if (fwrite(binaries_ptr[i], sizeof(unsigned char), binaries_size[i], f) != binaries_size[i])
...
// read binaries from files of file names "(program_file_name)_(name of device).bin"
UTILS_EXPORT
cl_program cl_util_read_binaries(const cl_context context, const cl_device_id * const devices, const cl_uint num_devices,
    const char * const program_file_name, cl_int * const error)
{
    ...
    binaries_size_alloc_size = sizeof(size_t) * num_devices;
    MEM_CHECK(binaries_size = (size_t *)malloc(binaries_size_alloc_size), err, end);

    // array for the binaries
    binaries_ptr_alloc_size = sizeof(unsigned char *) * num_devices;
    MEM_CHECK(binaries_ptr = (unsigned char **)malloc(binaries_ptr_alloc_size), err, end);
    memset(binaries_ptr, 0, binaries_ptr_alloc_size);
    ...
    for (cl_uint i = 0; i < num_devices; ++i) {
        // get device name
        char name_data[201];
        OCLERROR_RET(clGetDeviceInfo(devices[i], CL_DEVICE_NAME, sizeof(name_data) - 1, name_data, NULL), err, end);
        name_data[200] = '\0';
        // create input file name
        char filename[256];
        snprintf(filename, sizeof(filename), "%s-%s.bin", program_file_name, name_data);

        binaries_ptr[i] = cl_util_read_binary_file(filename, binaries_size + i, &err);
    ...
    OCLERROR_PAR(program = clCreateProgramWithBinary(context, num_devices, devices,
        binaries_size, binaries_ptr, &err, NULL), err, end);

...
template <> auto cl::sdk::parse<cl::sdk::options::Diagnostic>()
{
    return std::make_tuple(
        ...
        std::make_shared<TCLAP::SwitchArg>("q", "quiet",
    ...
    auto device_type = [](std::string in) -> cl_device_type {
        if (in == "all")
            ...
            throw std::logic_error{
                "Unkown device type after cli parse. Should not have happened."
            };
    };
...
template <> auto cl::sdk::parse<cl::sdk::options::Window>()
{
    return std::make_tuple(
        ...
        std::make_shared<TCLAP::ValueArg<int>>(
            "y", "height", "Height of window", false, 800, "positive integral"),

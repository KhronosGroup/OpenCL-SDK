...
do { err = func; if (err != CL_SUCCESS) { cl_util_print_error(err); fprintf(stderr,  "on line %d, in file %s\n%s\n", __LINE__, __FILE__, #func); goto label;} } while (0)

#define OCLERROR_PAR(func, err, label) \
do { func; if (err != CL_SUCCESS) { cl_util_print_error(err); fprintf(stderr,  "on line %d, in file %s\n%s\n", __LINE__, __FILE__, #func); goto label;} } while (0)

#define MEM_CHECK(func, err, label) \
do { if ((func) == NULL) {err = CL_OUT_OF_HOST_MEMORY; cl_util_print_error(err); fprintf(stderr,  "on line %d, in file %s\n%s\n", __LINE__, __FILE__, #func); goto label;} } while (0)
